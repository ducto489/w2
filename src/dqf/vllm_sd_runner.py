...
from dqf.sd_runner import NextTokenModel, RunResult, _profile_delta, _profile_snapshot
...
class DraftTargetProposerPair:
    target_model: NextTokenModel
    draft_proposer: DraftProposer


def run_speculative_decode_with_draft_proposer(
    ...
    for prompt in prompts:
        draft_profile_start = _profile_snapshot(pair.draft_proposer)
        ...
        while len(generated) < max_new_tokens:
            ...
            cycle_proposals = pair.draft_proposer.propose_tokens(working_context, gamma)
            draft_time_ms += (perf_counter() - draft_start) * 1000.0
            if len(cycle_proposals) != gamma:
                raise RuntimeError(
                    f"draft proposer returned {len(cycle_proposals)} tokens for gamma={gamma}"
                )
            proposed_tokens.extend(cycle_proposals)
        ...
        draft_profile = _profile_delta(
            draft_profile_start,
            _profile_snapshot(pair.draft_proposer),
        )
        ...
        record: dict[str, Any] = {
            ...
            "draft_model": pair.draft_proposer.name,
            ...
            "mean_accepted_length": compute_mean_accepted_length(
                accepted_tokens,
                speculation_steps,
            ),
            "acceptance_rate": compute_acceptance_rate(accepted_tokens, len(proposed_tokens)),
            "position_acceptance": [value / speculation_steps for value in position_acceptance],
            "draft_time_ms": draft_time_ms,
            "verify_time_ms": verify_time_ms,
            "total_wall_time_ms": total_wall_time_ms,
            "tokens_per_second": (
                output_count / total_wall_time_ms * 1000.0 if total_wall_time_ms > 0 else 0.0
            ),
            "quality_score": None,
            "notes": "vLLM draft proposer with HF target verification",
